...
Run: npx -y @anthropic-ai/claude-code mcp add npm @anthropic-ai/claude-mcp-server-stx402 --env WALLET_MNEMONIC="your 24-word seed phrase"
        ...
        <div className="mx-auto flex max-w-[1200px] items-center justify-between">
          ...
          >
            ...
            <Link
              ...
              className="text-white/85 transition-all duration-200 hover:text-white max-md:w-[280px] max-md:rounded-xl max-md:border max-md:border-white/10 max-md:bg-white/5 max-md:px-6 max-md:py-4 max-md:text-center max-md:hover:border-white/20 max-md:hover:bg-white/10"
            >
              Agents
            </Link>
            <Link
              href="/agents"
              onClick={() => setIsMenuOpen(false)}
              className="group inline-flex items-center justify-center gap-2 rounded-lg bg-[#F7931A] px-4 py-2 text-sm font-medium text-white transition-all duration-200 hover:bg-[#E8850F] hover:shadow-[0_0_20px_rgba(247,147,26,0.4)] active:scale-[0.97] max-md:w-[280px] max-md:rounded-xl max-md:py-3.5 max-md:text-base"
            >
              <span>Claim Your Agent</span>
            ...
            <div className="flex gap-3 overflow-x-auto px-12 pb-4 scrollbar-hide max-lg:px-8" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
              {featuredAgents.map((agent) => (
                <Link
                  href="/agents"
                  key={agent.id}
                  className="group flex-shrink-0 w-[180px] rounded-xl border border-white/[0.08] bg-gradient-to-br from-[rgba(26,26,26,0.6)] to-[rgba(15,15,15,0.4)] p-3.5 backdrop-blur-[12px] transition-all duration-200 hover:border-white/[0.15] hover:-translate-y-1"
                >
                  {/* Avatar */}
                  ...
                  <p className="text-[11px] leading-relaxed text-white/40 line-clamp-2">
                    {agent.description}
                  </p>
                </Link>
              ))}
