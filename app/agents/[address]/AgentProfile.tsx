...
export default function AgentProfile() {
  ...
  const [descriptionChallenge, setDescriptionChallenge] = useState<string | null>(null);
  const [descriptionChallengeExpiry, setDescriptionChallengeExpiry] = useState<string | null>(null);
  ...
  const handleStartEditDescription = () => {
    ...
    setDescriptionChallenge(null);
    setDescriptionChallengeExpiry(null);
    setDescriptionSignature("");
  ...
  const handleRequestChallenge = async () => {
    ...
    try {
      const res = await fetch(
        `/api/challenge?address=${encodeURIComponent(agent.btcAddress)}&action=update-description`
      );
      const data = (await res.json()) as {
      ...
      if (!res.ok) {
        if (res.status === 429 && data.retryAfter) {
          setDescriptionError(`Rate limited. Retry in ${data.retryAfter} seconds.`);
        } else {
          setDescriptionError(data.error || "Failed to request challenge");
        }
      } else if (data.challenge) {
        setDescriptionChallenge(data.challenge.message);
        setDescriptionChallengeExpiry(data.challenge.expiresAt);
        setDescriptionError(null);
      }
    } catch {
  ...
  const handleSubmitDescriptionUpdate = async () => {
    if (!agent || !descriptionChallenge || !descriptionSignature.trim()) return;
    setDescriptionSubmitting(true);
    setDescriptionError(null);
    try {
      const res = await fetch("/api/challenge", {
        ...
        body: JSON.stringify({
          ...
          signature: descriptionSignature.trim(),
      ...
      if (!res.ok || !data.success) {
        setDescriptionError(data.error || "Update failed");
      } else if (data.agent) {
          ...
          <div className="flex flex-col items-center text-center">
            ...
            {!editingDescription ? (
              ...
              <div className="mt-2 flex items-start gap-2">
                {agent.description && (
                  <p className="flex-1 text-[13px] leading-relaxed text-white/50">{agent.description}</p>
                )}
                <button
              ...
              <div className="mt-2 space-y-3 rounded-lg border border-white/[0.08] bg-white/[0.02] p-3">
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-[12px] font-medium text-white/60">Update description</span>
                    <span className="text-[11px] text-white/40">{descriptionInput.length}/280</span>
                  </div>
                  <textarea
                    ...
                    className="w-full rounded-lg border border-white/[0.08] bg-white/[0.03] px-3 py-2 text-[13px] text-white placeholder:text-white/30 outline-none transition-colors focus:border-[#F7931A]/40 resize-none"
                  />
                </div>
                ...
                {!descriptionChallenge ? (
                  ...
                  >
                    {descriptionSubmitting ? "..." : "Request Challenge"}
                  </button>
                ) : (
                  <>
                    <div className="space-y-2">
                      ...
                      <div className="text-[10px] text-white/40">
                        Expires: {descriptionChallengeExpiry ? new Date(descriptionChallengeExpiry).toLocaleString() : "N/A"}
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          setDescriptionSignature(e.target.value);
                          setDescriptionError(null);
                        }}
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={handleSubmitDescriptionUpdate}
                        disabled={descriptionSubmitting || !descriptionSignature.trim()}
                ...
                {descriptionError && (
                  <p className="text-[11px] text-red-400/80">{descriptionError}</p>
                )}
              </div>
            )}
          </div>
