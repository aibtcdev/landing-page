...
describe("status-filter totalCount uses live filtered count when stats drift (#906)", () => {
  function mockFilteredCount(n: number) {
    const first = vi.fn().mockResolvedValue({ n });
  ...
  it("status=unread returns filtered totalCount (not stale unreadCount stat)", async () => {
    mockFilteredCount(4);
    (listInboxMessagesFromD1 as Mock).mockResolvedValue([]);
    (getAgentInboxStats as Mock).mockResolvedValue({
      receivedCount: 50,
      unreadCount: 1, // drifted/stale
      sentCount: 0,
      lastMessageAt: null,
      lastSentAt: null,
    });
    ...
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.inbox.totalCount).toBe(4);
  });

  it("status=read returns filtered totalCount (not derived received-unread stat)", async () => {
    mockFilteredCount(7);
    (listInboxMessagesFromD1 as Mock).mockResolvedValue([]);
    (getAgentInboxStats as Mock).mockResolvedValue({
      receivedCount: 5,
      unreadCount: 5, // drift can force derived readCount to 0
      sentCount: 0,
      lastMessageAt: null,
      lastSentAt: null,
    });
    ...
    const res = await GET(
      buildGetRequest(AGENT_ADDR, "?status=read"),
      buildContext(AGENT_ADDR)
    );

    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.inbox.totalCount).toBe(7);
  });
});
