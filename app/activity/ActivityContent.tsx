...
/**
 * ActivityContent — client component for the /activity page.
 *
 * Fetches live data from /api/activity (refreshes every 30 s) and renders
 * the animated feed.  The static page shell (AnimatedBackground, Navbar) is
 * rendered by the parent Server Component so it is never included in the
 * client JS bundle.
 */
export default function ActivityContent() {
  ...
  // CSS media query already handles mobile row height via --feed-row-h variable,
  // so a fixed visible count is sufficient here.
  const VISIBLE_COUNT = Math.min(15, sourceEvents.length || 1);
...
function FullFeed({
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      setEnteringUid(uid);
      // Clear enteringUid after the animation completes (feedEnter 600ms +
      // glowFade start offset 100ms = ~700ms total).
      setTimeout(
        () => setEnteringUid((prev) => (prev === uid ? null : prev)),
        700
      );
      setItems((prev) => {
