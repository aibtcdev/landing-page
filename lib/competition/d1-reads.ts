...
/**
 * Display + identity columns joined from `agents`. The leaderboard surfaces
 * agent names alongside the per-sender aggregates rather than just STX
 * addresses, so this query joins agents in to avoid a follow-up N+1.
 */
export interface SwapWithAgent extends SwapRow {
...
interface D1SwapWithAgentRow extends D1SwapRow {
  display_name: string | null;
  bns_name: string | null;
  btc_address: string | null;
  erc8004_agent_id: number | null;
...
/**
 * Fetch every success-status swap joined with the agent's display fields.
 *
 * Used by the leaderboard cron — runs every 30 min, walks the full set,
 * aggregates in JS to produce the ranked snapshot. NOT meant for the
 * request path (no LIMIT, no pagination, no caching here — the snapshot
 * is the cache).
 *
 * Filters to `tx_status = 'success'` because the verifier already enforces
 * this gate; the WHERE clause is belt-and-suspenders in case a future
 * ingestion path writes a non-success row directly.
 */
export async function listSuccessSwapsWithAgentFromD1(
  db: D1Database
): Promise<SwapWithAgent[]> {
  const sql = `
    SELECT
      s.txid, s.sender, s.contract_id, s.function_name,
      s.token_in, s.amount_in, s.token_out, s.amount_out,
      s.burn_block_time, s.tx_status, s.source, s.scored_value, s.scored_at,
      a.display_name, a.bns_name, a.btc_address, a.erc8004_agent_id
    FROM swaps s
    JOIN agents a ON a.stx_address = s.sender
    WHERE s.tx_status = 'success'
    ORDER BY s.burn_block_time ASC
  `;
  const result = await db.prepare(sql).all<D1SwapWithAgentRow>();
  return (result.results ?? []).map((row) => ({
    ...mapSwapRow(row),
    display_name: row.display_name ?? null,
    bns_name: row.bns_name ?? null,
